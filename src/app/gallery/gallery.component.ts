...
import {
	...
	SceneService,
	UIService,
} from "projects/three/src/public-api";
...
export class GalleryComponent implements OnInit {
	...
	ngOnInit () {
		...
		this.frames = this.framesService.createFrames(
			this.artworks,
			this.buttons,
			afterSceneInitCB,
		);
		...
		this.selectedArtwork = signal( this.artworks[0], {
			equal: this.compareSelected,
		} );
	}
	 ...
	 * @param ind number frame index for where the buttons are 
	 * @param n: number direction, 1 for next, -1 for previous
	 */
